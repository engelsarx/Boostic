...
var paths = {
  dev: {
    ...
    svgIconsSprites: 'dev/images/icons-sprites/*.svg',
    svgScss: '../scss/icon-font/_icon-font.scss',
    svgFonts: '../fonts/',
    spritesSvgScss: './scss/sprites-svg/_sprites.scss',
...
gulp.task('sprites', function () {
  return gulp.src([paths.dev.svgIconsSprites])
    .pipe(svgSprite({
      svg: {
        sprite: "images/ibbc-sprites.svg"
      },
      common: 'sprite-icon',
      preview: false,
      padding: 10,
      templates: { css: false, scss: true },
      cssFile: paths.dev.spritesSvgScss
    }))
    .pipe(gulp.dest(paths.dev.root));
});
...
gulp.task('watch', ['browserSync'], function () {
  ...
  gulp.watch(paths.dev.kitAllFiles, ['sprites']);
